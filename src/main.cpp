...
void setup() 
{
  ...
  version = (VERSION_MAJOR * 100 + VERSION_MINOR * 10 + VERSION_BUILD);
  ...
  // OR restore default settings to EEPROM if "Menu +" switch is held closed at start of booting
  ...
  if (storedVersion != version)     // update EEPROM in case the value read doesn't match current version number
  {  
  ...
  //FastLED.addLeds<NEOPIXEL, DATA_PIN>(ledArray, totalLEDs);
  ...
  snprintf(buffer, 7, "%.1i%.1i%.1i--", VERSION_MAJOR, VERSION_MINOR, VERSION_BUILD);
...
void displayDigitalData()
{
  ...
  if ((minute(local) % 10) == 2 && second() == 15 && menuItem[randomMode].value == 1)     
  {
    ...
    if (menuItem[NixieMode].value == 1) randomDigits();                                  // Nixie Mode
    else randomDigits(hue);
  } 
...
/*******************************************************************************************************************************************
* Simulate Nixie anti-cathode poisoning random number display                   
********************************************************************************************************************************************/

// Custom color version

void randomDigits(uint8_t hsvHue)
{  
  ...
  for (uint8_t i = 0; i < 90; i++)
  {
    uint8_t address;
    uint8_t number;
    uint32_t rand = random(32768);

    // Clear the array so panels previously turned on won't show  
    for (address = 0; address < numModules * ledsPerModule; address++) ledArray[address] = CRGB(0, 0, 0);    

    number = ((rand % 10000) / 1000);              
    address = 0 * ledsPerModule + panelOrder[number];;                       
    ledArray[address] = displayColor;              // 10s hours
    if (displayType != 3) ledArray[address + 10] = displayColor;              
    
    number = ((rand % 1000) / 100) + 1;  
    if (number > 9) number = 0;                   
    address = 1 * ledsPerModule + panelOrder[number];                        
    ledArray[address] = displayColor;              // 1s hours
    if (displayType != 3) ledArray[address + 10] = displayColor;

    number = (rand % 1000) / 100;              
    address = 2 * ledsPerModule + panelOrder[number];                       
    ledArray[address] = displayColor;              // 10s minutes
    if (displayType != 3) ledArray[address + 10] = displayColor; 

    number = (rand % 100) / 10;                
    address = 3 * ledsPerModule + panelOrder[number];                      
    ledArray[address] = displayColor;              // 1s minutes
    if (displayType != 3) ledArray[address + 10] = displayColor; 

    number = rand % 10;                
    address = 4 * ledsPerModule + panelOrder[number];          
    ledArray[address] = displayColor;              // 10s seconds
    if (displayType != 3) ledArray[address + 10] = displayColor; 

    number = ((rand % 100) / 10) + 1; 
    if (number > 9) number = 0;         
    address = 5 * ledsPerModule + panelOrder[number];                      
    ledArray[address] = displayColor;              // 1s seconds 
    if (displayType != 3) ledArray[address + 10] = displayColor;     
...
{  
  for (uint8_t i = 0; i < 90; i++)
  {
    ...
    // Clear the array so panels previously turned on won't show and set Nixie aura 
    for (address = 0; address < numModules * ledsPerModule; address++) ledArray[address] = scaledNixieAura;  
    
    number = ((rand % 10000) / 1000);              
    address = 0 * ledsPerModule + panelOrder[number];;                       
    ledArray[address] = nixieIon;              // 10s hours
    if (displayType != 3) ledArray[address + 10] = nixieIon;   
    
    number = ((rand % 1000) / 100) + 1;  
    if (number > 9) number = 0;                   
    address = 1 * ledsPerModule + panelOrder[number];                        
    ledArray[address] = nixieIon;              // 1s hours
    if (displayType != 3) ledArray[address + 10] = nixieIon; 

    number = (rand % 1000) / 100;              
    address = 2 * ledsPerModule + panelOrder[number];                       
    ledArray[address] = nixieIon;              // 10s minutes
    if (displayType != 3) ledArray[address + 10] = nixieIon; 

    number = (rand % 100) / 10;                
    address = 3 * ledsPerModule + panelOrder[number];                      
    ledArray[address] = nixieIon;              // 1s minutes
    if (displayType != 3) ledArray[address + 10] = nixieIon;  

    number = rand % 10;                
    address = 4 * ledsPerModule + panelOrder[number];          
    ledArray[address] = nixieIon;              // 10s seconds
    if (displayType != 3) ledArray[address + 10] = nixieIon; 

    number = ((rand % 100) / 10) + 1; 
    if (number > 9) number = 0;         
    address = 5 * ledsPerModule +  panelOrder[number];                      
    ledArray[address] = nixieIon;              // 1s seconds 
    if (displayType != 3) ledArray[address + 10] = nixieIon;     
    FastLED.show(); 
    delay(10 + i * 2); 
  }
...
/*******************************************************************************************************************************************
* Set parameters for the digit modules and OLED                     
********************************************************************************************************************************************/
...
void setDisplayType()
{
  ...
  case 0b11:
    ...
    //display.flipScreenVertically(); // flip for Nick's
    ledsPerModule = 10;
  ...
  case 0b10:
    ...
    panelOrder[0] = 3;              // LED address to light the 0 panel on the first Lixie.
    panelOrder[1] = 4; 
    panelOrder[2] = 2; 
  ...
  case 0b01:
    ...
    panelOrder[0] = 5;              // LED address to light the 0 panel on the first EleksTube. 
    panelOrder[1] = 0; 
    panelOrder[2] = 6; 
...
/********************************************************************************************************************************************
* Called when TimeLib wants to sync with time server, tries ten times                     
********************************************************************************************************************************************/
...
void menuSettings()
{
  ...
  menuItem[TZinfo].lowLimit = version;
  menuItem[TZinfo].highLimit = version;
  menuItem[TZinfo].increment = 0;
  menuItem[TZinfo].value = version;
