...
void setup() 
{
  ...
  iVersion = strtol(cVersion, NULL, 10);                                          // integer representation of version                                                    
  sprintf(cVersion, "%i.%i.%i", VERSION_MAJOR, VERSION_MINOR, VERSION_BUILD);     // char array representation with dots 
 ...
 readEEPROM();                    // now read stored settings into menu struct 
  ...
  // Starting state for the blue LED, on or off depending on the menu setting 
  // If value is set to 0 (Mode A) the following statement will turn the LED off 
  // with an ESP module where the LED is wired active high
  // Setting the value to 1 (Mode B) does the reverse
  // If the LED is wired active low on the ESP module the operation would also be reversed
  digitalWrite(BUILT_IN_LED, menuItem[syncLED].value); 
  ...
  displayVersion();                  // shows version on digit modules, also provides a 2 second delay
  ...
  else
  {
    ...
    digitalWrite(BUILT_IN_LED, menuItem[syncLED].value);   // turn off the LED or vice versa depending on menu setting
...
void loop()
{
  ...
  // Turn off sync LED if it's 10 seconds before next sync, or vice versa depending on menu setting
  if (currentMillis - ledOnMillis > (SYNC_INTERVAL - 10) * 1000) digitalWrite(BUILT_IN_LED, menuItem[syncLED].value);    
  ...
  if (pic > picMax)         // normal time/date/temperature display, if pic <= picMax allow menu button functions to display menu items
  {
    ...
    if (nightMode(menuItem[nightBeginHour].value, menuItem[nightEndHour].value)) FastLED.setBrightness(menuItem[nightBrightness].value);   // night brightness
...
void updateEEPROM()
{
  ...
  for (uint8_t i = 1; i <= picMax; i++)
  {
    ...
    if (oldValue != menuItem[i].value)
    {
      ...
      if (i > 18 && i < 21) setColonLevels();         // check and update colon levels
...
{ 
  char buffer[36];
  snprintf(buffer, 36, "%s v%s", displayName, cVersion);
...
void displayMenuItem()
{
  ...
  FastLED.setBrightness(menuItem[nightBrightness].value);
  digitModuleWrite(buffer, CRGB::Green);    
...
void setTimeZone()
{
  ...
  while (!loc.status && count < 3)     // try three times to connect to geoip server
  {
    ...
    // Any of these calls to geoIP work depending on whether you have a key for ipapi.co  
    // and if you want the results shown on the serial monitor 
    ...
    //loc = geoip.getGeoFromWiFi("put your key here", true);
    //loc = geoip.getGeoFromWiFi("put your key here", false);   
    //loc = geoip.getGeoFromWiFi("put your key here");  
  } 
...
time_t getNtpTime()
{
  ...
  while (millis() - beginWait < 1500) 
  {
    ...
    if (size >= NTP_PACKET_SIZE) 
    {
      ...
      // If Sync LED is set to Mode A in the menu the value is 0 so the following statement will light the LED
      // on an ESP module where the LED is wired active high
      // Mode B sets the value to 1 so it will light the LED on an ESP module where the LED is wired active low
      digitalWrite(BUILT_IN_LED, !menuItem[syncLED].value); 
...
void menuSettings()
{
  ...
  strncpy(menuItem[nightBrightness].name, "Night brightness", 17);
  menuItem[nightBrightness].lowLimit = 0;
  menuItem[nightBrightness].highLimit = 255;
  menuItem[nightBrightness].increment = 15;
  menuItem[nightBrightness].value = 30; 
  ...
  strncpy(menuItem[syncLED].name, "Sync LED", 9);
  strncpy(menuItem[syncLED].settingName[0], "Mode A", 7);
  strncpy(menuItem[syncLED].settingName[1], "Mode B", 7);
  menuItem[syncLED].lowLimit = 0;
  menuItem[syncLED].highLimit = 1;
  menuItem[syncLED].increment = 1;
  menuItem[syncLED].value = 0;   
...
/*******************************************************************************************************************************************
* Show the version on edge-lit displays                        
********************************************************************************************************************************************/
...
void displayVersion()
{
  ...
  if (VERSION_MAJOR > 99) val = VERSION_MAJOR % 100;
  else val = VERSION_MAJOR;
  uint8_t tens = val / 10;
  uint8_t ones = val % 10;
  if (tens == 0) buffer[0] = 45;    // ASCII '-' to blank the digit
  else buffer[0] = tens + 48;       // ASCII values for numerals are 48 bigger than the numeral they represent 
  buffer[1] = ones + 48;
  
  if (VERSION_MINOR > 99) val = VERSION_MINOR % 100;
  else val = VERSION_MINOR;
  tens = val / 10;
  ones = val % 10;
  if (tens == 0) buffer[2] = 45;
  else buffer[2] = tens + 48;
  buffer[3] = ones + 48;

  if (VERSION_BUILD > 99) val = VERSION_BUILD % 100;
  else val = VERSION_BUILD;
  tens = val / 10;
  ones = val % 10;
  if (tens == 0) buffer[4] = 45;
  else buffer[4] = tens + 48;
  buffer[5] = ones + 48;
    
  buffer[6] = '\0';

  ledArray[totalLEDs - 2] = CRGB::SlateGrey;
  ledArray[totalLEDs - 1] = CRGB::SlateGrey;      
