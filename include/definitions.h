...
#define DATA_PIN (GPIO_NUM_4)                // LED data 
//#define DATA_PIN (GPIO_NUM_13)               // Alternate LED data pin
#define MENU_INCREMENT_BUTTON (GPIO_NUM_19)  // Active low 19
#define MENU_DECREMENT_BUTTON (GPIO_NUM_23)  // Active low 23
#define DATE_TEMP_BUTTON (GPIO_NUM_18)       // Active low 18, toggles between auto and manual temperature and date display
...
#define OLED_FLIP (GPIO_NUM_27)              // Connect to ground to flip the text if display is mounted with pins at bottom
...
// Global variables - Yes, I know global variables should be avoided but there are valid reasons to use these!
// Mainly because several functions need to access or modify these values and this is the most straight-forward way to do it.
...
char           displayName[21]; 
const uint8_t  picMax = 33;                      // the number of highest menu item                    
uint8_t        pic = picMax + 1;                        
uint16_t       version;
uint8_t        totalLEDs; 
uint16_t       displayType;                      // 0 - Lixie II, 1 - EleksTube, 2 - Lixie, 3 - NixieCron                     
uint8_t        numModules;
